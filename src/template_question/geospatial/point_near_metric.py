...
def make_question_point_near_metric(df_osm, ratio=None, list_distance=[100, 300, 500, 1000], nb_q=110, seed=42):
    ...
    # deux marges sont donc équilibrées, et la somme des cellules vaut max(1, nb_q // len(list_distance)).
    ...
    for (cat_anc, cat_q), nb_cell in allocate(max(1, nb_q // len(list_distance)), poids).items():
        if not nb_cell:
            continue
