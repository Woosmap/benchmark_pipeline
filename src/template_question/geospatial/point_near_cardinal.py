...
def make_question_point_near_cardinal(df_osm, ratio=None, list_direction=["north", "east", "west", "south"], half_width=70.0, nb_q=110, seed=42):
    ...
    """
    if nb_q <= 20:
        raise ValueError("nb_q doit être ≥ 20 ...")
    ...
    for cat_anc, nb_q_anc in balance.items():
        ...
        anchors = anchors.sample(n=min(nb_q_anc//len(list_direction), len(anchors)), random_state=rng)
